...
class HomeController extends Controller
{
    ...
    public function merchant_register()
    {
        ...
        return view('auth.merchant_register', ['merchants'=>$merchants, 'countries'=>$countries]);
    }
    ...
    public function index()
    {
        ...
        return view('frontend.home', ['products_top'=>$products_top, 'products_featured'=>$products_featured, 'promotions'=>$promotions, 'featured_categories'=>$featured_categories,
    ...
    public function profile()
    {
      ...
      return view('frontend.profile', ['lvl'=>$lvl, 'totalBalance'=>$totalBalance, 'totalEarn'=>$totalEarn,
    ...
    {
        $totalBalance = $this->GetWalletBalance();
        $totalPoint = $this->GetPointBalance();
        ...
        return view('frontend.wallet', ['all'=>$all, 'totalBalance'=>$totalBalance, 'withdrawlHistorys'=>$withdrawlHistorys,
                                        'banks'=>$banks, 'banksDefault'=>$banksDefault, 'lvl'=>$lvl, 'totalPoint'=>$totalPoint], compact('purchaseDetail'));
    }
    ...
    public function bank_account_edit($id)
    {
      ...
      return view('frontend.bank_account', ['bank'=>$bank, 'lvl'=>$lvl]);
    }
    ...
    public function save_wallet(Request $request)
    {
      ...
      if(floatval($request->wallet_balance) < floatval($amount)){
          // return 123;
      ...
      $check = WithdrawalTransaction::where('user_id', Auth::user()->code)
                                      ...
                                      ->first();
        if(!empty($check->id)){
            return Redirect::back()->withInput(Input::all())->withErrors('You have a new withdrawal request that has not been approved / rejected by the film, please wait for the administrator to review');
        }
    ...
    public function GetWalletBalance()
    {
        ...
        $totalBalance = $balance->totalBalance - $withdrawal->totalWithdrawal - $transaction->totalUsedPoint;
    ...
    public function listing()
    {
        ...
        return view('frontend.listing', ['products'=>$products, 'categories'=>$categories, 'brands'=>$brands, 'count_p'=>$count_p, 'sp_products'=>$sp_products, 'MaxMinPrice'=>$MaxMinPrice], 
                                        compact('favourite', 'priceV', 'productImages', 'top_subCategory'));
    }
    ...
    public function point_listing()
    {
        ...
        $productImages = [];
        foreach($products as $product){
            $variations = ProductVariation::select(DB::raw("max(variation_point_price) AS maxVPrice"),
                                                   DB::raw("max(variation_point_agent_price) AS maxVAPrice"),
                                                   DB::raw("min(variation_point_price) AS minVPrice"),
                                                   DB::raw("min(variation_point_agent_price) AS minVAPrice"))
                                          ->where('product_id', $product->id)
                                          ->where('variation_name', '!=', '')
                                          ->first();
            $priceV[$product->id] = [$variations->maxVPrice, $variations->minVPrice, $variations->maxVAPrice, $variations->minVAPrice];
        ...
        return view('frontend.point_listing', ['products'=>$products, 'categories'=>$categories, 'brands'=>$brands, 'count_p'=>$count_p, 'sp_products'=>$sp_products, 'MaxMinPrice'=>$MaxMinPrice], 
                                               compact('favourite', 'priceV', 'productImages'));
    }
    ...
    public function point_details($name, $id)
    {
        if(Auth::guard('web')->check()){
          $user = Auth::guard('web')->check();
          $userCode = Auth::guard('web')->user()->code;
        ...
        return view('frontend.point_details', ['product'=>$product, 'stockBalance'=>$stockBalance, 'images'=>$images, 'favourite'=>$favourite, 
                                         'products'=>$products, 'Pimage'=>$Pimage, 'Ppackages'=>$Ppackages,
                                         'sub_category_id'=>$sub_category_id, 'variations'=>$variations], compact('vStock'));
    }
    ...
    public function checkout()
    {
        ...
        return view('frontend.checkout', ['carts'=>$carts, 'states'=>$states, 'shipping_address'=>$shipping_address, 'totalBalance'=>$totalBalance, 'banks'=>$banks, 'checkAppliedPromo'=>$checkAppliedPromo,
                                          'getClaimedPromos'=>$getClaimedPromos, 'totalshipping_fees'=>$totalshipping_fees]);
    }
    ...
    public function placeOrder(Request $request){
        ...
        if(empty($request->mall) && $request->mall != '1'){
          if($request->cdm == 1){
              ...
              $input['processing_fee'] = number_format($totalPFee, 2);
        ...
        if(!empty($request->mall) && $request->mall == '1' || !empty($request->cdm) && $request->cdm == '1'){
          ...
          }elseif(Auth::guard('web')->check() || Auth::guard('admin')->check() || Auth::guard('merchant')->check()){
            Toastr::success('Order Successfully');
    ...
    public function payment_successfully(Request $request)
    {
        ...
        //     $update = Transaction::where('transaction_no', $request->refid);
        //     $update = $update->update(['status'=>'1']);
        ...
        //               $totalPoint = $SettingMerchantRebate->amount;
        //           }else{
        //               $totalPoint = $amount * $SettingMerchantRebate->amount / 100;
        //           }
    ...
    public function AgentUpgrade($user_id)
    {
        ...
        if(!empty($merchant->id)){
          ...
          $detail = Merchant::where('code', $user_id)->first();
    ...
    public function sendEmailNotification($to, $from, $name, $subject, $transaction_no)
    {
      ...
      $body .= "<td style='border:none;'><strong>Hello Threestar Admin!</strong></td></tr>";
      $body .= "<tr><td style='border:none;'><strong>you've a new order #".$transaction_no."</strong></td></tr>";
      $body .= "<tr><td>Regards,</td></tr>";
      $body .= "<tr><td>Three Star</td></tr>";
      $body .= "<tr><td></td></tr>";
