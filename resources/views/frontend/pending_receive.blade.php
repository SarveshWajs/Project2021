			...
			<div class="row justify-content-center">
				...
				<div class="col-xs-2" align="center">
					<a href="{{ route('pending_receive') }}" style="position: relative;">
						...
						<br>
				<span class="profile-word" style="font-weight: bold; text-decoration: underline;">To Receive</span>
					</a>
				</div>
					...
					<div class="row">
						...
						<div class="col-xs-6" align="right">
							...
							@else
								...
								<a href="#" class="btn btn-primary btn-sm complete-order" data-id="{{ $transaction->id }}">
									Complete
								</a>
							@endif
						</div>
...
<script type="text/javascript">
	$('.complete-order').click( function(e){
		...
		var fd = new FormData();
        	fd.append('tid', ele.data('id'));
          if(confirm('Complete this order?') == true){
	        $.ajax({
	           url: '{{ route("update_complete") }}',
	           type: 'post',
	           data: fd,
	           contentType: false,
	           processData: false,
	           success: function(response){
	                window.location.href="{{ route('completed_order') }}";
	           },
	        });        	
        }
	});
